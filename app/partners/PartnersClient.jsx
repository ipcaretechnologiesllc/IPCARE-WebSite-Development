            ...
            {filtered.map((p, i) => (
              <div key={p.slug} className="glass-card p-7 reveal" style={{ transitionDelay: `${i * 50}ms` }}>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="mono text-[10px] uppercase tracking-widest px-2 py-1 rounded-full" style={{ background: `${tierColor[p.tier]}22`, border: `1px solid ${tierColor[p.tier]}66`, color: tierColor[p.tier] === '#C0C0C0' ? '#e8e8e8' : tierColor[p.tier] }}>{p.tier}</span>
                </div>
