...
// Gallery sizes: the main image fills 3/5 of the 1300px grid on desktop, full width
// (minus padding) below lg; thumbnails are a third of that.
const MAIN_SIZES = '(min-width: 1024px) 740px, calc(100vw - 48px)'
const THUMB_SIZES = '(min-width: 1024px) 240px, 30vw'
      ...
      <div className="max-w-[1300px] mx-auto grid lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-3">
          <div className="overflow-hidden aspect-[4/3] relative mb-4 rounded-2xl" style={{ background: '#FFFFFF', border: '1px solid rgba(11,26,70,0.08)', boxShadow: '0 8px 32px rgba(10,26,70,0.10)' }}>
            <img src={images[activeImage]} srcSet={srcSetFor(images[activeImage])} sizes={MAIN_SIZES} alt={`${product.brand} ${product.model}`} className="w-full h-full object-contain p-6"/>
          ...
          <div className="grid grid-cols-3 gap-3">
            {images.map((img, i) => (
              <button key={i} onClick={() => setActiveImage(i)} className={`aspect-[4/3] rounded-lg overflow-hidden transition ${activeImage === i ? 'ring-2 ring-[#E87722]' : 'opacity-70 hover:opacity-100'}`} style={{ background: '#FFFFFF', border: '1px solid rgba(11,26,70,0.08)' }}>
                <img src={img} srcSet={srcSetFor(img)} sizes={THUMB_SIZES} alt="" loading="lazy" className="w-full h-full object-contain p-2"/>
              </button>
            ))}
          </div>
        </div>
