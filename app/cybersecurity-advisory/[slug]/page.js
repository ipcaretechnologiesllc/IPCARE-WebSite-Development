...
export default function AdvisorySubPage({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: sub.h1, item: process.env.NEXT_PUBLIC_BASE_URL + `/cybersecurity-advisory/${params.slug}` },
    ],
  }
  const serviceSchema = {
    '@context': 'https://schema.org',
    ...
    name: sub.h1,
    description: sub.metaDescription,
    provider: { '@type': 'Organization', name: 'IP Care Technologies L.L.C. — The Cyber Adviser', url: process.env.NEXT_PUBLIC_BASE_URL },
