...
Allow **5–10 minutes** after pushing before verifying — a check immediately after `git push` will still show the old build. `www.ipcare.ae` is served by Hostinger; the bare apex `ipcare.ae` is redirected to `www` by a Cloudflare Redirect Rule at the edge, so apex requests never reach Hostinger. See "Hosting & DNS topology" below for the DNS path.
...
**Apex → www is a Cloudflare Single Redirect, not Hostinger.** Rules → Redirect Rules → "Apex ipcare.ae -> www.ipcare.ae": `(http.host eq "ipcare.ae")` → `concat("https://www.ipcare.ae", http.request.uri.path)`, 301, query string preserved; http and https apex both reach `www` in one hop. It was added 2026-09-24 because on 2026-09-10 Hostinger's apex vhost served its LiteSpeed 503 on every path while `www` was healthy, so the platform-level redirect is not something to rely on. Hostinger's own apex 308 and the `ipcare.ae` entry in `next.config.js` `redirects()` are now unreachable backups; leave them in place. When editing this ruleset through the API, a PUT on the `http_request_dynamic_redirect` phase entrypoint **replaces every rule** in it, so GET the existing rules first and send them back with your change. Use an exact `http.host eq` match: a `contains "ipcare.ae"` expression also matches `www` and creates a redirect loop.
...
**Responsive image variants.** `images.unoptimized: true` means nothing resizes images at request time, so large `/public` images have committed `-480w.webp` / `-960w.webp` siblings made by `node scripts/gen-responsive-images.mjs`, listed in `lib/image-variants.json`. `responsive(src, SIZES.x)` / `srcSetFor(src)` from `lib/responsive-image.js` turn that into `srcSet`/`sizes` (and build Unsplash `?w=` srcsets). After adding or replacing a large image, re-run the script and commit the variants + JSON — it only processes git-tracked files, so commit the original first. **Don't preload heroes with react-dom `preload()` in a page:** Next sends those hints in its link-prefetch payload, so every page linking to that page downloaded its hero (seen in PageSpeed on the homepage). Give the hero `<img>` `fetchPriority="high"` (not lazy) and React preloads it — with its srcset — in that page's HTML only. If a `<link rel="preload">` is needed (e.g. media-split heroes in `app/page.js`), render it as JSX and pass matching `imageSrcSet`/`imageSizes`.
...
- `next.config.js` `redirects()` — host-based 308 redirects for legacy domains (`ipcare.ae`, `ipcares.com` → `www.ipcare.ae`; the `ipcare.ae` entry is only a backup, since the Cloudflare edge redirect answers apex requests first), plus a large block of permanent redirects mapping legacy `.php`/`.html`/WordPress URLs to current routes.
