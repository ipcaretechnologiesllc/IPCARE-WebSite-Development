...
// `focusService` (optional): a service href such as '/services/elv/cctv-systems'.
// Portfolio projects that include it are listed first, so a CCTV page leads with
// CCTV installations rather than the general ELV mix.
export function getLocalProof(category, city, focusService) {
  const cityName = UAE_CITIES[city]
  if (!cityName) return null

  const inCity = (loc) => (loc || '').includes(cityName)
  const hasFocus = (p) => (p.services || []).some((s) => s.href === focusService)
  const byFocus = (list) => focusService ? [...list.filter(hasFocus), ...list.filter((p) => !hasFocus(p))] : list
  const cityEvents = linkedFirst(events.filter((e) => inCity(e.location)).map(eventCard))
  const cityProjects = linkedFirst(byFocus(portfolioProjects.filter((p) => inCity(p.location))).map(projectCard))

  let projects
  if (city === 'abu-dhabi' && focusService) {
  ...
  return {
    ...
    // A focused page (not the category's own city page) also links back to that page.
    siblings: Object.keys(CITY_SERVICE_LABELS)
      .filter((c) => focusService || c !== category)
