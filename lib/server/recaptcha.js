...
export async function verifyRecaptchaToken(token, { action, threshold = DEFAULT_THRESHOLD, remoteip } = {}) {
  ...
  //   - development → BYPASS, so forms still work locally before keys are loaded.
  //   - production  → FAIL CLOSED. Previously this bypassed everywhere, which meant a
  //     deploy that dropped RECAPTCHA_SECRET_KEY silently disabled bot protection on
  //     every form with no visible signal. A hard failure is noisy but safe.
  if (!secret) {
    if (process.env.NODE_ENV === 'production') {
      console.error('[recaptcha] RECAPTCHA_SECRET_KEY is not set in production — rejecting submission')
      return { ok: false, score: 0, error: 'captcha-misconfigured' }
    }
